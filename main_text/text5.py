...
def watershed_algorithm(image):
    ...
    # ret, surface = cv2.threshold(dist_out, 0.14*dist_out.max(), 255, cv2.THRESH_BINARY)
    ...
    # window_size = 11
    # sigma = 5
    # local_mean = cv2.GaussianBlur(dist_out, (window_size, window_size), sigma)
    # mask = dist_out > local_mean
    # threshold_value = dist_out[mask].mean()
    # ret, surface = cv2.threshold(dist_out, threshold_value, 255, cv2.THRESH_BINARY)
    ...
    print(threshold_value/dist_out.max())
    # 使用自适应阈值处理
    # surface = cv2.adaptiveThreshold(dist_out, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 11, 2)
    # cv2.imshow('surface', surface)
    ...
    result = cv2.addWeighted(src,0.1,image,0.9,0) #图像权重叠加
    cv2.imshow('result', result)

src = cv2.imread('_Archive/2y-08WC-SDS1h-12PAH2h-wc30m-07.tif')
cv2.imshow('src', src)
