...
def knn_merge(segmented_images, area_thresh_pixels=1):
    ...
    for i, img in enumerate(segmented_images):
        ...
        if num_pixels > area_thresh_pixels:
            large_regions.append((i, num_pixels))
        else:
            small_regions.append((i, num_pixels))

    # 执行KNN区域合并
    large_centroids = np.array([np.mean(np.where(img), axis=1) for i, _ in large_regions])
    nbrs = NearestNeighbors(n_neighbors=5, algorithm='ball_tree').fit(large_centroids)
    
    for i, _ in small_regions:
        small_centroid = np.mean(np.where(segmented_images[i]), axis=1).reshape(1, -1)
        _, indices = nbrs.kneighbors(small_centroid)
        
        # 将小区域合并到最近的大区域
        nearest_large_region = large_regions[indices[0, 0]]
        segmented_images[nearest_large_region[0]][segmented_images[i] > 0] = nearest_large_region[1]
        
    # 组合所有图像
    combined_image = np.zeros_like(segmented_images[0])
...
def watershed_algorithm(image):
    ...
    # 使用knn_merge函数进行合并
    combined_markers = knn_merge(combined_markers)
