...
def watershed_algorithm(image):
    ...
    combined_markers = np.uint8(combined_markers)
    # 对combined_markers进行形态学操作
    combined_markers = cv2.morphologyEx(combined_markers, cv2.MORPH_OPEN, kernel)

    # # 使用膨胀和腐蚀操作消除小区域并与相邻的前景合并
    # kernel = np.ones((3, 3), np.uint8)
    # combined_markers = cv2.dilate(combined_markers, kernel, iterations=1)
    # combined_markers = cv2.erode(combined_markers, kernel, iterations=2)
