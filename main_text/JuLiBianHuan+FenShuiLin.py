...
def watershed_algorithm(image):
    ...
    # cv2.imshow('distance-Transform', dist_out * 50)

    # 对距离变换图像进行二值化处理，得到前景区域的初始标记，0.5*dist_out.max()表示阈值
    ret, surface = cv2.threshold(
        dist_out, 0.5*dist_out.max(), 255, cv2.THRESH_BINARY)
    ...
    # cv2.imshow('morphologyEx', binary)
    ...
    for i in range(2, int(max_val+1)):
        ...
        cv2.drawContours(black_bg, contours, -1, colors[(i - 2) % 12], -1)
    ...
    # cv2.putText(src, "count=%d" % (int(max_val-1)),
    #             (220, 30), 0, 1, (0, 255, 0), 2)
    # cv2.putText(image, "count=%d" % (int(max_val-1)),
    #             (220, 30), 0, 1, (0, 255, 0), 2)

    # 将原图和分割后的图像进行叠加，得到可视化显示的结果
    # cv2.imshow('regions', image)
    # result = cv2.addWeighted(src, 0.5, image, 0.5, 0)  # 图像权重叠加
    result = cv2.addWeighted(src, 0.5, black_bg, 0.5, 0)
