...
class SegmentationConnectObject(object):
    ...
    def main_find_mindist_points(self):
        ...
        """=>>利用Harris进行角点检测<<= """
        harris = cv2.cornerHarris(open, 3, 5, 0.04)     #<== 进行角点检测,blockSize:角点检测中要考虑的领域大小||ksize - Sobel:求导中使用的窗口大小||k - Harris:角点检测方程中的自由参数, 取值参数为[0, 04, 0.06]
        ...
        pix_max = 0.1 * harris.max()                      # <==获取选择角点的阈值
    ...
    def connect_domain(self):
        ...
        for i in range(1, len(centers)):
            cv2.drawMarker(result, (int(centers[i][0]), int(centers[i][1])), (0, 0, 255), 1, 10, 2)
...
#     h, s, v = cv.split(hsv)
#     ret, binary = cv.threshold(v, 100, 255, cv.THRESH_BINARY)
#     #
...
# 使用自适应阈值而不是固定阈值来考虑不同的照明条件：分割效果好，但噪声多
def get_binary(img):
    ...
    binary = cv.adaptiveThreshold(blurred, 255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY, 11, 2)

    return binary

# 使用组合要删除噪音小和灰色污渍:分割效果好，但噪声多
# def get_binary(img):
#     #Convert the image to grayscale
#     gray = cv.cvtColor(img, cv.COLOR_BGR2GRAY)
...
#     ret, binary = cv.threshold(blurred, 100, 255, cv.THRESH_BINARY)

#     #Use morphological operations to remove small noise and gray stains
...
#     # Invert the binary image if necessary (make sure the white particles are the foreground)
#     if np.mean(binary) > 127:
#         binary = cv.bitwise_not(binary)

#     # Perform morphological operations to remove small noise and enhance the particles
...
#     upper_gray = np.array([180, 80, 150])
...
#     # Set the range for gray stains in the HSV space
#     lower_gray = np.array([0, 0, 0])
#     upper_gray = np.array([180, 80, 150])
...
#     # Perform bitwise AND operation on the original image and mask
#     res = cv.bitwise_and(img, img, mask=~mask)
...
#     # Morphological operation, dilate first and then erode (closing), remove small noise points while preserving edges
#     kernel = np.ones((3, 3), np.uint8)
#     binary = cv.morphologyEx(binary, cv.MORPH_CLOSE, kernel, iterations=1)
